...
/obj/item/card/id/syndicate/attack_self(mob/user as mob)
	if(!src.registered)
		...
		var/color = input(user, "What color should the ID's color band be?\nClick cancel to abort the forging process.") as null|anything in list("clown","magician","golden","blue","red","green","purple","yellow","No band")
		switch (color)
			if ("clown")
				src.icon_state = "id_clown"
			if ("magician")
				src.icon_state = "id_magician"
			if ("golden")
				src.icon_state = "gold"
			if ("No band")
				src.icon_state = "id"
